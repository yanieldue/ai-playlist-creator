...
LANGUAGE FILTER (hard rule): When the user's request references artists who perform in a specific language (e.g. English-language American or British artists), ONLY include songs performed in that same language. This is a hard reject — if a song is performed in French, Hindi, Portuguese, Spanish, Korean, Italian, Arabic, or any other non-matching language, exclude it regardless of how well it matches the style or vibe. Do NOT include non-English songs just to reach the target count — it is better to return fewer songs than to include language mismatches. Only include non-English songs if the user explicitly requests international/multilingual content or names non-English artists.
