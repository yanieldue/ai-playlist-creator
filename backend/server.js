...
async function enrichCatalogWithAudioFeatures(songs, maxSongs = 40) {
  ...
  // Pass 1: check song_details first — if fully populated, skip re-fetch
  const songDetailsMap = songs.filter(s => s.uuid).length > 0
    ? await db.getSongDetailsByUuids(songs.filter(s => s.uuid).map(s => s.uuid))
    : new Map();
  const needsFetch = [];
  for (const song of songs) {
    if (!song.uuid) continue;
    const row = songDetailsMap.get(song.uuid);
    // Only skip fetch if song_details already has audio AND moods AND genres
    if (row?.energy != null && row?.moods?.length && row?.genres?.length) {
      const audio = { energy: row.energy, valence: row.valence, danceability: row.danceability,
        tempo: row.tempo, loudness: row.loudness, acousticness: row.acousticness,
        instrumentalness: row.instrumentalness, speechiness: row.speechiness,
        liveness: row.liveness, key: row.key_signature, mode: row.mode, timeSignature: row.time_signature };
      enriched.set(song.uuid, { audio, moods: row.moods || [], themes: row.themes || [] });
    } else {
...
IMPORTANT — You MUST select at least ${Math.floor(_targetCount * 0.7)} songs and ideally close to ${_targetCount}. Selecting fewer than ${Math.floor(_targetCount * 0.5)} is a FAILURE. If most candidates are already the correct genre, be generous with inclusion — only reject clear mismatches (wrong genre, wrong language). Style/vibe preferences should influence your RANKING of candidates, not cause mass rejection. When in doubt, include rather than exclude.
...
Language filter: When the user's request references artists who perform in a specific language (e.g. English-language American or British artists), only include songs performed in that same language. Reject songs you KNOW are performed in a non-matching language (French, Hindi, Portuguese, Spanish, Korean, Italian, Arabic, etc.) — for example, PLK and Nekfeu rap in French, DIVINE raps in Hindi, Djonga raps in Portuguese. However, if you don't know what language a song is in, or the artist primarily performs in the expected language, include it. Do not reject songs from well-known English-language artists just because you're uncertain. Only include non-English songs if the user explicitly requests international/multilingual content or names non-English artists.

Anchor artist interpretation: When the user says "like [Artist A] and [Artist B]", PRIORITIZE songs that match the specific subgenre and style those artists represent, not just the broad genre. For example, "like Kendrick Lamar and J. Cole" means prefer conscious, lyrical, introspective hip hop over commercial pop-rap or trap bangers. But this is a RANKING preference, not a hard filter — if the pool doesn't have enough conscious/lyrical tracks to reach the target count, include well-known tracks from the broader genre rather than drastically undershooting. The target count (${_targetCount}) is more important than perfect style matching.
